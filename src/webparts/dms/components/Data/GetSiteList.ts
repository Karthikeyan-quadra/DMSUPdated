...
export async function getEditSitelist() {
  ...
  const items: any[] = await sp.web.lists
    .getByTitle("User Files")
    .items.filter(
      `ApprovalStatus eq 'APPROVER 2' or ApprovalStatus eq 'APPROVER 3'`
    )
    .getAll();
  return items;
}
...
export async function Get_departmentusers(Dept) {
  ...
  return await sp.web.lists
    ...
    .items.filter(`Department eq '${Dept}'`)
    .getAll();
}
...
export async function getUserDetails() {
  ...
  const items: any[] = await sp.web.lists
    ...
    .items.filter(`EmailID eq '${await (await sp.web.currentUser()).Email}'`)
...
export async function getInitialApprovers(Department, Level, SubDepartment) {
  ...
  if (SubDepartment == "")
    items = await sp.web.lists
      .getByTitle("Approverlist")
      .items.filter(`Department eq '${Department}' and Level eq '${Level}' `)
      .getAll();
  else
    items = await sp.web.lists
      .getByTitle("Approverlist")
...
export async function getQMSApprover() {
  ...
  let items = await sp.web.lists
    .getByTitle("Final Appover")
    .items.filter(`ApproverLevel eq 'APPROVER 4'`)
    .getAll();
  console.log(items);
  return items;
}
  ...
  let req = a.map(
    async (val) =>
      await sp.web.lists
        ...
        .items.filter(`ParentFolders eq '${val.Departments}'`)
        .getAll()
  );

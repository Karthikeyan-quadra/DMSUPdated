    ...
    const Approvemail: any = async (value, ApprovalStatus) => {
      ...
      try {
        ...
        if (ApprovalStatus === "APPROVER 2") {
          ...
          const sp: SPFI = getSp();
          const buffer: ArrayBuffer = await sp.web.getFileByServerRelativePath(`${value.RelativeURL}/${value.Filename}`).getBuffer().then((res: any) => res
          ).catch(e => console.error(e))

          console.log(`${value.RelativeURL}/${value.Filename}`);
          console.log(buffer);
          const blob: Blob = await sp.web.getFileByServerRelativePath(`${value.RelativeURL}/${value.Filename}`).getBlob().then((res: any) => res
          ).catch(e => console.error(e))
          ...
          const exists: any = await sp.web.getFolderByServerRelativePath(`${destinationUrl}`).files.getByUrl(`${value.Filename}`).exists().then((res) => res
          ).catch(e => console.error(e)
          )
          console.log(exists);
          ...
          if (exists === true) {
            ...
            await sp.web.getFolderByServerRelativePath(`${destinationUrl}`).files.addUsingPath(value.Filename, blob, { Overwrite: true })
              .then(async (file) => {

...
// console.log(fileContent);
    ...
    const folderPath: any = "/sites/DMSportal/Shared Documents/ADMINISTRATION/GENERAL ADMIN/SOP/";

    const folder:any = await sp.web.getFolderByServerRelativePath(folderPath);
    const files = await folder.files.getAll();
    
    console.log(files);
    // You can further process the files here
...
const files:any = await folder.files.getAll().then(async (s) => {
    ...
    // const folderPath = `/sites/DMSportal/Shared Documents/${fileUrl}`;
    // const folder:any = await sp.web.getFolderByServerRelativePath(folderPath);
    // const files:any = await folder.files.getAll() .then(async (item) => {
    //        console.log(item);
    //        if(item.length > 0) {
...
try {
  ...
  // You might want to set an appropriate state or log the error for debugging
  // this.setState({ error: 'An error occurred while fetching files.' });
}
    ...
    try {
        ...
        if (files.length > 0) {
          ...
          const downloadUrl = firstFile.FileRef;
      
          this.setState({
            fileUrl: somee.join("/"),
            Filess: files.map((file) => ({ key: file.Name, text: file.Name })),
...
const uploadedFile: any = await folder.files.add(`${this.state.fileNameStruct}.${fileexe}`, myfile, true);

// Get the corresponding item
const item: any = await uploadedFile.file.getItem();

// Update metadata and retrieve the item details
await item.update({
  FileDescription: this.state.fileDes,
...
const updatedItem: any = await item.get();

// Log the updated item details
console.log(updatedItem);

// Update state with the file ID
await this.setState({
  fileIDs: updatedItem.ID,
});
        ...
        const fold: any = await sp.web.getFolderByServerRelativePath(foldPath);
        const files: any = await fold.files.getAll();
    ...
    //     });
      ...
      const sp:SPFI=getSp()
      await console.log(myfile.name);
      let fileexe:any = myfile.name.split(".")[1];
      await sp.web.lists
        ...
        .then((f) => {
          ...
          f.file.getItem().then(async(item) => {
            ...
            // Unable to solve the getAll error
